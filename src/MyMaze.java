...
public class MyMaze {
    ...
    public static MyMaze makeMaze(int rows, int cols) {
        ...
        while (!(s.isEmpty())) {
            ...
            Cell current = m.maze[nextPossiblelevel[0]][nextPossiblelevel[1]];
            ...
            if (options.length > 0) {//so if the length is greater than 0 we KNOW that we have options to move into
                ...
                current.setVisited(true);//we are going to se the current cell to be true
                ...
                if (options[randomIndex] == 1) {//East
                    ...
                    current.setRight(false);//we go into the right and set right to be true
        ...
        while (!(q.isEmpty())) {
            ...
            if (frontElement[0] == (this.rows - 1) && frontElement[1] == (this.cols - 1)) {
                ...
                if (frontElement[1] < this.cols) {// this is for east
                    ...
                    if (east.getVisited() == false && current.getRight() == false) {
                        int[] eastOfElement = new int[2];
                        eastOfElement[0] = frontElement[0];
                        eastOfElement[1] = frontElement[1] + 1;

                        q.add(eastOfElement);//pushes the array onto the queue.
                        // The functionality of this is nearly identical for each other direction
                ...
                if (frontElement[0] > 1) {//south case
                    ...
                    if (south.getVisited() == false && current.getBottom() == false) {
